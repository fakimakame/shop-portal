...
const productSlice = createSlice({
    ...
    reducers: {
        ...
        updateProductState(state, action) {
            ...
            state.product[index] = newUpdatedProduct

        },
        updateProductAfterAddedToSample(state, action) {
            const result = action.payload
            const { message, payload } = result
            const index = state.product.findIndex((element: any) => element.id === payload.product.id)
            const currentProduct = state.product[index]
            const { quantity, ...currentProductOnStringfy } = JSON.parse(JSON.stringify(currentProduct))
    ...
    extraReducers(builder) {
        builder.addCase(onViewProduct.pending, (state, action) => {
            ...
            .addCase(onDeleteProduct.fulfilled, (state, action) => {
                ...
                state.product = state.product.filter((res: any) => res.id !== action.payload.id)
                state.isLoading = false
...
export const { updateProductState, updateProductAfterAddedToSample } = productSlice.actions
