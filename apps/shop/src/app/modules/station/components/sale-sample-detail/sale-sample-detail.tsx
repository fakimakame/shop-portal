import styles from './sale-sample-detail.module.scss';
...
/* eslint-disable-next-line */
export interface SaleSampleDetailProps {
  changeStatus?: any,
  status?: boolean,
  existingData?: any,
  currentSample?: any
...
export function SaleSampleDetail(props: SaleSampleDetailProps) {
  ...
  const update = async () => {
    setIsLoading(true)
    await productSizeService.addQuantitiesToCart(initialSizeData, props.existingData.proId, props.currentSample.id)
    //  dispatch(updateProductAfterAddedToSample(productData))
    setIsLoading(false)
    handleClose()
  }
  const handleText = (event: React.ChangeEvent<HTMLInputElement>, id: number) => {
    const availableStockData = sizeData.find((element: sizeModel) => element.id === id)
    ...
    if (/^\d*$/.test(newValue)) {
      ...
      const updatedQty = availableStockData?.qty ? availableStockData.qty : 0
      ...
      if (updatedQty < totalAfterAddingNewValue) {
        ...
        setMsg(`Exceed available stock of ${availableStockData?.qty} `)
        setStatus(true)
      }
      else {
        setMsg('')
        ...
        const updatedSizeData = [
          ...initialSizeData.slice(0, index),
          { ...initialSizeData[index], qty: parseInt(newValue) },
          ...initialSizeData.slice(index + 1),
        ];
        setInitialSizeData(updatedSizeData)
        setValue(newValue)
      }
  ...
  const increaseQuantity = (id: number) => {
    ...
    const getTotalQuantity = (id: number): number => {
      return initialSizeData.reduce((total: number, item: sizeModel) => {
        return item.id === id ? total + item.qty : total;
      }, 0);
    }
    //geting available quantity plus new value
    ...
    const index = sizeData.findIndex((element: sizeModel) => element.id === id)
    const updatedQty: number = availableStockData?.qty ? availableStockData?.qty : 0 // + (initialData?.qty || 0) //nimefika hapa
    ...
    if (updatedQty < totalAfterAddingNewValue) {
      ...
      const updatedSizeData = [
        ...initialSizeData.slice(0, index),
        { ...initialSizeData[index], qty: initialSizeData[index].qty + 1 },
        ...initialSizeData.slice(index + 1),
      ];
      setInitialSizeData(updatedSizeData)
      setMsg('')
  ...
  const decreaseQuantity = (id: number) => {
    ...
    const index = initialSizeData.findIndex((element: sizeModel) => element.id === id)

    if (initialSizeData[index].qty > 0) {
      const newUpdateSize = [
        ...initialSizeData.slice(0, index),
        { ...initialSizeData[index], qty: initialSizeData[index].qty - 1 },
        ...initialSizeData.slice(index + 1)
      ]
      setInitialSizeData(newUpdateSize)
      setMsg('')
      setQuantity(prev => prev - 1);
      setStatus(false)
  ...
  const getSize = async () => {
    const data: sizeModel[] = await productSizeService.getStoreProductSize(props.existingData.proId, props.currentSample.id)
    setInitialSizeData(data)
    const updatedSizeData: sizeModel[] = []
    data.forEach((element) => {
      const { qty, ...rest } = element
      const payload = {
        ...rest,
        qty: 0
      }
      updatedSizeData.push(payload)
      //  updatedSizeData = [
      //     ...initialSizeData.slice(0, index),
      //     { ...initialSizeData[index], qty: 0 },
      //     ...initialSizeData.slice(index + 1),
      //   ];
    ...
    >
      ...
      <DialogContent>
        <ul role="list" className="divide-y divide-gray-100">
          {
            sizeData.map((size: sizeModel, index: number) => (
                ...
                <div className="tw-flex tw-w-">
                  <div className="flex items-center space-x-2">
                    ...
                    <input
                      ...
                      name="first-name"
                      type="text"
                      value={initialSizeData[index] ? initialSizeData[index].qty : 0}
...
export default SaleSampleDetail;
